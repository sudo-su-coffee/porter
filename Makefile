...
# backend/web/dist, since main.go embeds it via go:embed.
backend:
	cd backend && go build -trimpath -ldflags "-s -w -X main.Version=$(VERSION)" -o porter .
...
# and proxies API calls to a `go run .` backend. Run these in two
# separate terminals; this target just starts the backend.
dev:
	cd backend && go run .
