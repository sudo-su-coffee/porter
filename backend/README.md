...
- **Preview & production domains + a real authoritative DNS server.** `internal/dns/server.go` is a wire-level authoritative DNS server (miekg/dns-based) for `*.baseDomain` zones, listening on UDP/TCP 53. `internal/dns/domains.go` auto-assigns preview/prod domains on project creation. **[DONE — Stage 2a/2b]**
- **TLS via ACME (Let's Encrypt).** `internal/tls/autocert.go` provides automatic certificate management via `golang.org/x/crypto/acme/autocert` with HTTP-01 challenges, cert caching, and daily renewal. **[DONE — Stage 2c]**
