...
type API struct {
	...
	// Single prototype admin account (from porter.toml [admin], no
	// user database). /login checks these and, on success, hands back
	// the same token every other endpoint expects on the
	// Authorization header — the login screen gates access to that
	// token, it doesn't replace the existing bearer-token API auth.
	adminUser string
	adminPass string
}

func NewAPI(store *Store, hub *Hub, vmm *VMManager, net *NetManager, token, baseDomain, adminUser, adminPass string) *API {
	return &API{
...
// Routes registers every endpoint from ARCHITECTURE.md §2 on a stdlib
// net/http.ServeMux (Go 1.22+ pattern routing — no external router dep
// needed for this constrained API surface).
...
// --- Login ---
//
// There's no user database — one admin username/password lives in
// porter.toml (or PORTER_ADMIN_USERNAME/PORTER_ADMIN_PASSWORD). This
// endpoint only exists to gate the dashboard's login screen; every
// other route still authenticates the same way it always did, via the
// static bearer token this handler hands back on success.
...
func (a *API) handleDeleteVM(w http.ResponseWriter, r *http.Request) {
	...
	if vm.State == StateRunning || vm.State == StateBooting {
		a.vmm.Stop(vm)
	}
	a.store.DeleteVM(id)
...
func (a *API) registerStableDomain(vm *VM) {
	if a.baseDomain == "" {
		return
	}
	host := vm.Name
	...
	a.store.AddDomain(vm.ID, &Domain{
...
func (a *API) handleAddDomain(w http.ResponseWriter, r *http.Request) {
	...
	d := &Domain{Domain: req.Domain, Type: "custom", Status: "pending"}
...
func (a *API) handleCreateComposeProject(w http.ResponseWriter, r *http.Request) {
	...
	svcs, err := ParseCompose(req.ComposeYAML)
	if err != nil {
...
func (a *API) bootService(proj *Project, svc ComposeService) {
	pool := &ServicePool{Desired: svc.Replicas}
...
func (a *API) handleScale(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		if req.Replicas > current {
			// scale up: boot new replicas at the next available indices
			var sampleImage string
			var sampleEnv map[string]string
			var samplePorts []Port
			var sampleHC *Healthcheck
			if len(pool.VMs) > 0 {
			...
			for i := current; i < req.Replicas; i++ {
				vm := &VM{
					ID: newID(), Name: fmt.Sprintf("%s-%s-%d", proj.Name, service, i),
					ProjectID: proj.ID, ServiceName: service, State: StatePending,
					HealthStatus: HealthChecking, ReplicaIndex: i, Image: sampleImage,
