...
type stubStore struct{ vms []*types.VM }

func (s *stubStore) GetVM(id string) (*types.VM, bool) {
	for _, v := range s.vms {
		if v.ID == id {
			return v, true
		}
...
func (s *stubStore) ListVMs() []*types.VM { return s.vms }

func vm(id, svc, project, ip string) *types.VM {
	return &types.VM{ID: id, ServiceName: svc, ProjectID: project, Name: project, IPAddress: ip}
}

func TestLookupIP(t *testing.T) {
	r := New(&stubStore{vms: []*types.VM{
		vm("1", "web", "myapp", "10.0.0.5"),
		vm("2", "db", "myapp", "10.0.0.6"),
	}})
...
func TestLookupIPUnknownService(t *testing.T) {
	r := New(&stubStore{vms: []*types.VM{vm("1", "web", "myapp", "10.0.0.5")}})
