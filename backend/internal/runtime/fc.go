...
// --api-sock <path>` opens. It replaces firecracker-go-sdk on purpose:
// only stdlib is needed for six small PUT requests.
...
// kernel command line (also how the guest's static IP is injected).
...
// InstanceStart boots the configured VM (everything above just loads
// config into the not-yet-running machine).
func (c *FCClient) InstanceStart(ctx context.Context) error {
	return c.put(ctx, "/actions", fcAction{ActionType: "InstanceStart"})
}

// SendCtrlAltDel asks the guest kernel to shut down cleanly.
...
// accepts a connection, or ctx is done.
