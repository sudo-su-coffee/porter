...
const error = ref("");
const purgeAllEndpoint = computed(() => `/projects/${projectId}/cache/purge`);
const purgePathEndpoint = computed(() => `/projects/${projectId}/cache/purge/path`);
...
async function purge(endpoint, body) {
  busy.value = true;
  try { await api(endpoint, { method: "POST", ...(body ? { body: JSON.stringify(body) } : {}) }); toast(endpoint === purgeAllEndpoint.value ? "Project cache purged" : "Cache path purged", "success"); path.value = ""; await load(); }
