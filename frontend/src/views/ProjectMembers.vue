...
async function inspect(member) {
  const name = username(member);
  try { selected.value = await api(`/projects/${projectId}/members/${encodeURIComponent(name)}`); }
