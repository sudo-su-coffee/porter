...
const saveMethod = computed(() => section.value === "general" ? "PATCH" : "PUT");
const values = ref({});
const rows = ref([]);
const error = ref("");
...
<template>
  ...
  <header class="page-header"><div><div class="page-title">{{ title }}</div><div class="page-sub">This form reads and writes the persisted <code>/projects/:projectId/settings/{{ section }}</code> contract.</div></div><div class="detail-actions"><button class="btn btn-sm" :disabled="loading || busy" @click="load">Refresh</button><button class="btn btn-primary btn-sm" :disabled="loading || busy" @click="save">{{ busy ? "Saving…" : `Save ${section}` }}</button></div></header>
  ...
  <section v-else class="card"><div class="card-title">{{ section }} fields</div><p v-if="!rows.length" class="page-sub">No saved values exist for this section yet. Add only settings supported by your deployment policy.</p><div v-for="(row, index) in rows" :key="index" class="field-row settings-row"><div class="field"><label>Key</label><input v-model="row.key" placeholder="setting_name" /></div><div class="field field-wide"><label>Value</label><textarea v-model="row.value" rows="2" placeholder="value or JSON" /></div><button class="btn btn-danger btn-sm" @click="removeRow(index)">Remove</button></div><div class="detail-actions" style="margin-top:16px"><button class="btn btn-sm" @click="addRow">Add field</button><button class="btn btn-primary btn-sm" :disabled="busy" @click="save">Save settings</button></div></section>
</template>
