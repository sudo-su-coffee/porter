#!/usr/bin/env bash
# Porter production installer.
#
# This is the only production entrypoint. When run from a Porter checkout it
# delegates to the internal source installer. When downloaded or piped from
# GitHub it downloads and verifies the published release package, reuses the
# verified local cache, asks for PostgreSQL setup, and delegates to the package
...
if [ -x "$SCRIPT_DIR/install-porter.sh" ] && [ -s "$SCRIPT_DIR/porter" ]; then
  exec "$SCRIPT_DIR/install-porter.sh" "$@"
fi
...
# Fetching the tiny checksum sidecar on each run detects a rebuilt release
# under the same tag without redownloading the large package on a cache hit.
...
if [ -s "$ARCHIVE" ] && (cd "$CACHE_DIR" && printf '%s  %s\n' "$EXPECTED" "$PACKAGE" | sha256sum -c - >/dev/null 2>&1); then
  ...
  printf 'Downloaded and cached verified release archive: %s\n' "$ARCHIVE"
fi
...
[ -x "$TMP_DIR/package/install.sh" ] || die "release package did not contain install.sh"
if [ "$POSTGRES_MODE" = remote ]; then
  PORTER_RELEASE_ARCHIVE="$ARCHIVE" PORTER_RELEASE_PACKAGE_SHA256="$EXPECTED" \
  PORTER_POSTGRES_MODE="$POSTGRES_MODE" PORTER_DATABASE_URL="$PORTER_DATABASE_URL" \
    "$TMP_DIR/package/install.sh" "$ARCH"
else
  PORTER_RELEASE_ARCHIVE="$ARCHIVE" PORTER_RELEASE_PACKAGE_SHA256="$EXPECTED" \
